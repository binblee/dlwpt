...
class LunaTrainingApp:
    ...
    def initDataLoader(self, is_val=False) -> DataLoader:
        train_ds = LunaDataset(val_stride=10, isValSet_bool=is_val, data_path=self.cli_args.data_path)
    ...
    def doTraining(self, epoch_ndx, train_dl):
        ...
        log.info('start batch loading ({})'.format(len(train_dl)))
        batch_iter = enumerateWithEstimate(
            train_dl,
            'E{} training '.format(epoch_ndx),
            start_ndx=train_dl.num_workers
        )
        log.info('end batch loading ({})'.format(len(train_dl)))
